...
class Activity(models.Model):
    ...
    vr_cards = models.ManyToManyField(VRCard, blank=True)
    def get_content_filename(self):
        return os.path.basename(self.content.name)
...
class ActivityVRCard(models.Model):
    activity = models.ForeignKey(Activity, on_delete=models.CASCADE)
    vr_card = models.ForeignKey(VRCard, on_delete=models.CASCADE)

    def __str__(self):
        return f"{self.activity.title} - {self.vr_card.title}"

class RespuestaActividad(models.Model):
    actividad = models.ForeignKey(Activity, on_delete=models.CASCADE)
    archivo_respuesta = models.FileField(upload_to='respuestas/',blank=True, null=True)  # Campo para el archivo de respuesta

    def __str__(self):
        return f'Respuesta de {self.actividad}'
...
class ResourceContent(models.Model):
    ...
    content_type = models.CharField(max_length=10, choices=content_type_choices)
    content = models.FileField(upload_to='resource_content/', blank=True, null=True)

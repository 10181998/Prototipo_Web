...
from .forms import ModuleForm,LessonForm,ContentForm,ClassActivityForm,VRActivityForm,ResourceContentFormSet
...
def module_detail_instructor(request, module_id):
    ...
    activities = module.activity_set.all()

    # Recuperar los contenidos asociados a cada actividad
    for activity in activities:
        activity.contents = activity.contents.all()
...
@login_required
def delete_lesson_instructor(request, lesson_id):
    lesson = get_object_or_404(Lesson, id=lesson_id)
    module_id = lesson.module.id  # Obtener el ID del módulo antes de eliminar la lección
    if request.method == 'POST':
        lesson.delete()
        ...
        return redirect('module_detail_instructor', module_id=module_id)
    return render(request, 'dashboard/instructor/delete_lesson_instructor.html', {'lesson': lesson})


# Vista para crear una actividad

from .forms import  ClassActivityForm, VRActivityForm,ActivityForm
    ...
    if request.method == 'POST':
        ...
        if activity_form.is_valid():
            ...
            if activity.activity_type == 'clase':
                class_form = ClassActivityForm(request.POST, request.FILES, instance=activity)
                if class_form.is_valid():
                    class_form.save()
            elif activity.activity_type == 'realidad_virtual':
                vr_form = VRActivityForm(request.POST, instance=activity)
                if vr_form.is_valid():
                    vr_form.save()
...
"""def create_activity_instructor(request, module_id):
    ...
    return render(request, 'dashboard/instructor/create_activity_instructor.html', {'form': form,'module': module})


def create_class_activity(request, module_id):
    module = get_object_or_404(Module, id=module_id)
    if request.method == 'POST':
        form = ClassActivityForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            return redirect('module_detail_instructor', module_id=module.id)  
    else:
        form = ClassActivityForm()
    
    return render(request, 'class_activity_form.html', {'form': form,'module': module})

def create_vr_activity(request,module_id):
    module = get_object_or_404(Module, id=module_id)
    if request.method == 'POST':
        form = VRActivityForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('module_detail_instructor', module_id=module.id)  # Reemplaza 'ruta_a_redirigir' con la URL adecuada
    else:
        form = VRActivityForm()
    
    return render(request, 'vr_activity_form.html', {'form': form,'module': module})
...
def create_resource_instructor(request,module_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            resource = form.save()
            # Obtiene los contenidos seleccionados desde el formulario
            selected_contents = form.cleaned_data['contents']

            # Asigna los contenidos seleccionados al recurso
            resource.contents.set(selected_contents)

            return redirect('module_detail_instructor', module_id=module_id)
    else:
        form = ResourceForm()
    
    return render(request, 'dashboard/instructor/create_resource_instructor.html', {'form': form,'module': module})
"""
...
def create_resource_instructor(request, module_id):
    ...
    if request.method == 'POST':
        formset = ResourceContentFormSet(request.POST, queryset=Resource.objects.none())
        
        if formset.is_valid():
            instances = formset.save(commit=False)
            
            # Iterar sobre los recursos creados para asignar los contenidos
            for instance in instances:
                selected_contents = formset.cleaned_data.get(formset.prefix + '-contents')
                if selected_contents:
                    instance.contents.set(selected_contents)
        ...
        formset = ResourceContentFormSet(queryset=Resource.objects.none())
    
    return render(request, 'dashboard/instructor/create_resource_instructor.html', {'formset': formset, 'module': module})
...
"""def edit_resource_instructor(request, resource_id,module_id):
    ...
    return render(request, 'dashboard/instructor/edit_resource_instructor.html', {'resource_form': resource_form, 'resource': resource,'module': module})

def delete_resource_instructor(request, resource_id,module_id):
    module = get_object_or_404(Module, id=module_id)
    resource = get_object_or_404(Resource, id=resource_id)

    if request.method == 'POST':
        resource.delete()
        return redirect('module_detail_instructor', module_id=module_id)  # Redirige a la lista de recursos después de eliminar

    return render(request, 'dashboard/instructor/delete_resource_instructor.html', {'resource': resource,'module': module})
"""
...
def module_list_learner(request, codigo):
    course = get_object_or_404(Course, codigo=codigo)
    modules = Module.objects.filter(course=course) # Obtener los módulos asociados al curso
    return render(request, 'dashboard/learner/detalle_curso_instructor.html', {'course': course, 'modules': modules})
